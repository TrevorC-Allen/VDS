...
- Phase 12 后续 correctness + GPT-like hardening 已新增语义正确性红线：点名文件时指标/维度必须绑定在该文件内，`产品/门店/城市/客户` 等语义不能串；`利润率 / margin / rate` 必须按 `sum(profit)/sum(sales)` 等派生 ratio 执行或明确澄清；可信 join 才能 materialize，不可信 join 必须提示候选关联键和风险；overview 必须区分可计算事实表、维表、说明或元数据表。`success=true` 不能绕过语义错算；已修能力不得下降 / non-regression 是测试门禁。
