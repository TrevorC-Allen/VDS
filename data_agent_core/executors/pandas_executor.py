...
def _top_k_share(df: pd.DataFrame, filters: dict[str, Any], params: dict[str, Any]) -> float | list[dict[str, Any]]:
    ...
    use_derived_ranking = bool(
        ...
        and (str(ranking_metric) == derived_metric_name or str(ranking_metric) not in data.columns)
    )
    use_derived_share = bool(
        derived_metric
        and share_metric not in {None, "__row_count__", "row_count", "transaction_count"}
        and (str(share_metric) == derived_metric_name or str(share_metric) not in data.columns)
    )
    derived_grouped: pd.Series | None = None
    if use_derived_ranking or use_derived_share:
        derived_rows = _aggregate_derived_ratio_grouped(data, dimension, derived_metric)
        derived_grouped = pd.Series(
            {row.get(dimension): float(row.get(derived_metric_name or "ratio") or 0.0) for row in derived_rows},
            dtype="float64",
        )
    ...
    elif use_derived_ranking and derived_grouped is not None:
        ranking_grouped = derived_grouped.sort_values(ascending=False)
    else:
    ...
    elif use_derived_share and derived_grouped is not None:
        denominator = float(derived_grouped.sum())
        numerator = float(derived_grouped.loc[[value for value in selected_groups if value in derived_grouped.index]].sum())
        per_group_numerators = derived_grouped.to_dict()
    else:
