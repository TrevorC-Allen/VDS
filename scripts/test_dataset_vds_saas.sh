...
OUT_DIR="${OUTPUT_ROOT}/${RUN_ID}-vds_saas_generic"
...
VDS_LLM_PROVIDER="${REFERENCE_LLM_PROVIDER}" "${PY}" scripts/run_generic_dataset_eval.py \
  --dataset-name vds_saas \
  --files "${DATA_ROOT}/QueryGPT_SaaS订阅数据_单表版.xlsx" \
  "${STANDARD_ARGS[@]}" \
  ${VDS_ARGS[@]+"${VDS_ARGS[@]}"} \
  --output-dir "${OUT_DIR}" \
  --print-summary

echo "VDS SaaS comparison: ${OUT_DIR}/comparison.md"
echo "VDS SaaS answers: ${OUT_DIR}/vds_answers.jsonl"
...
echo "VDS SaaS scored comparison: ${OUT_DIR}/comparison_scored.md"
