...
OUT_DIR="${OUTPUT_ROOT}/${RUN_ID}-vds_logistics_generic"
...
VDS_LLM_PROVIDER=mock "${PY}" scripts/run_generic_dataset_eval.py \
  --dataset-name vds_logistics \
  --files "${DATA_ROOT}/QueryGPT_物流配送数据_单表版.xlsx" \
  ${VDS_ARGS[@]+"${VDS_ARGS[@]}"} \
  --output-dir "${OUT_DIR}" \
  --print-summary

echo "VDS logistics comparison: ${OUT_DIR}/comparison.md"
echo "VDS logistics answers: ${OUT_DIR}/vds_answers.jsonl"
...
echo "VDS logistics scored comparison: ${OUT_DIR}/comparison_scored.md"
